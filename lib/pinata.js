...
export async function uploadJsonToPinata(jsonData, name) {
  const response = await fetch('https://api.pinata.cloud/pinning/pinJSONToIPFS', {
    ...
    headers: {
      ...
      // Consistent with your file upload function
      'Authorization': `Bearer ${process.env.PINATA_JWT}` 
    },
    body: JSON.stringify({
  ...
  return {
    ipfsHash: result.IpfsHash,
    // Use the same .env gateway for consistency
    ipfsUrl: `${process.env.PINATA_GATEWAY}/${result.IpfsHash}`
  };
}
...
export function createARC3Metadata({ 
  ...
  category,
  creator,
  ...otherData // This collects any extra fields without nesting them
}) {
  ...
  return {
    ...
    // Uses your NEXT_PUBLIC_BASE_URL from the .env
