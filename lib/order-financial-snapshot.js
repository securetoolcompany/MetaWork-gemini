...
function buildLockedLicenseTermsSnapshot(terms = []) {
  ...
  return terms.map((term) => {
    ...
    if (
      !normalizeId(term?.ipAssetId) ||
      !Number.isInteger(licensingFeeCents) ||
      licensingFeeCents < 0 ||
      !Number.isInteger(Number(term?.platformFeeBps)) ||
      Number(term.platformFeeBps) < 0
    ) {
      throw new Error(
    ...
    return {
      ipAssetId: normalizeId(term.ipAssetId),
      licensingFeeCents,
      platformFeeBps: Number(term.platformFeeBps),
      requiresSettlement: Boolean(term.requiresSettlement),
...
export function buildOrderTotalsSnapshot({
  merchandiseSubtotal,
  shipping,
  tax,
  discount = 0,
  ...
  const merchandiseSubtotalCents = toCents(
    merchandiseSubtotal,
    'merchandiseSubtotal'
  );

  const shippingCents = toCents(shipping, 'shipping');
  const taxCents = toCents(tax, 'tax');
  const discountCents = toCents(discount, 'discount');

  const customerTotalCents =
    merchandiseSubtotalCents + shippingCents + taxCents - discountCents;

  if (customerTotalCents < 0) {
    throw new Error(
      '[order-financial-snapshot] customerTotalCents cannot be negative'
    );
  }
