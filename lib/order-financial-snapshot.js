...
export function buildOrderItemSnapshot({
  ...
  return {
    ...
    lockedProductRevenuePool: (() => {
      ...
      if (
        pool?.tokenizationStatus !== 'active' ||
        !pool?.poolKey ||
        !Number.isSafeInteger(Number(pool?.revenuePoolAppId)) ||
        Number(pool.revenuePoolAppId) <= 0 ||
        !Number.isSafeInteger(Number(pool?.revenueTokenAssetId)) ||
        Number(pool.revenueTokenAssetId) <= 0
      ) {
        return null;
      }
