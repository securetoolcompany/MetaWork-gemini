...
assert.equal(firstBatch.poolKey, 'pool-paid-a');
...
assert.equal(secondBatch.poolKey, 'pool-paid-b');
...
const zeroFeeDb = createFakeDb();

zeroFeeDb.rows.push({
  _id: 'ledger-zero',
  orderId: 'order-zero',
  idempotencyKey: 'order-zero:item-1:ip-zero',
  status: 'release_eligible',
  settlementBatchId: null,
  settlementLeaseId: null,
  settlementLeaseExpiresAt: null,
  ...
  revenueTokenAssetId: 8003,
  allocationCents: 0,
  usdcAtomicUnits: 0,
  createdAt: new Date('2026-08-09T19:03:00.000Z'),
  stateTransitions: [],
});

const zeroFeeBatch = await createSettlementBatchFromEligibleRows({
  db: zeroFeeDb,
  orderId: 'order-zero',
  now,
});

assert.equal(zeroFeeBatch, null);

const zeroFeeLedger = zeroFeeDb.rows.find(
  (row) => row._id === 'ledger-zero'
);

assert.equal(zeroFeeLedger.status, 'release_eligible');
assert.equal(zeroFeeLedger.settlementBatchId, null);
assert.equal(zeroFeeLedger.settlementLeaseId, null);
assert.equal(zeroFeeDb.batches.length, 0);
...
conflictDb.batches.push({
  _id: 'conflicting-batch',
    batchKey:
    'app:7001:pool:pool-paid-a:token:8001:b97c273070b976d0189ae516a258e2ce4c592af21230addefdc7f75966fd946e',
  status: 'created',
  revenuePoolAppId: 7001,
  poolKey: 'pool-paid-a',
  revenueTokenAssetId: 8001,
