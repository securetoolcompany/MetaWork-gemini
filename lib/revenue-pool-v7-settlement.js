...
export function getV7RoundBoxName(ipAssetId, roundId) {
  ...
  const prefix = new TextEncoder().encode(`rnd_${normalizedIpAssetId}`);
  const boxName = new Uint8Array(prefix.length + 8);
  ...
  new DataView(
    ...
    boxName.byteOffset,
    boxName.byteLength
  ).setBigUint64(prefix.length, BigInt(normalizedRoundId), false);
