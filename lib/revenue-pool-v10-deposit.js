...
export const V10_DEPOSIT_USDC_ACTION = 'deposit_usdc';
...
export function buildV10DepositUsdcGroup({
  ...
  const depositUsdcAppCallTxn =
    algosdk.makeApplicationNoOpTxnFromObject({
      ...
      appIndex: appId,
      appArgs: [
        new TextEncoder().encode(V10_DEPOSIT_USDC_ACTION),
...
export function getV10PreparedGroupMetadata({
  ...
  if (getAction(appCallTxn) !== V10_DEPOSIT_USDC_ACTION) {
  throw new V10DepositValidationError(
    `V10 app call action must be "${V10_DEPOSIT_USDC_ACTION}"`,
