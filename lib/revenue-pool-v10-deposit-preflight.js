...
function getGlobalStateEntry(application, key) {
  ...
  return globalState.find((entry) => {
    ...
    return entryKey === encodedKey || entryKey === key;
  }) ?? null;
}
...
function decodeAdminAddress(application) {
  ...
  if (typeof encodedValue !== 'string' || !encodedValue) {
    throw new TypeError(
      'V10 application admin value must be base64 address bytes',
    );
  }
  ...
  return decodeAddressBytes(bytes, 'V10 application admin');
}
