    ...
    if (
      !(
        (typeof index === 'number' &&
          ...
          index >= 0) ||
        (typeof index === 'bigint' && index >= 0n) ||
        (typeof index === 'string' && /^\d+$/u.test(index))
      )
    ) {
      return false;
    }
...
function createFreshnessSnapshot({
  ...
  return Object.freeze({
    ...
    appAdminAddress,
    proxyAddress,
    appAccountUsdcAtomicUnits: appUsdcBalance.toString(),
...
export function preflightV10DepositUsdc({
  ...
  let appUsdcBalance = null;
  let appUsdcHoldingReadFailed = false;

  if (assetId !== null) {
    try {
      appUsdcBalance = getAssetHolding(appAccount, assetId);
    } catch (error) {
      appUsdcHoldingReadFailed = true;
      reasons.push(
        reason('APP_USDC_HOLDING_INVALID', error.message),
      );
    }
  }

  if (!appUsdcHoldingReadFailed && appUsdcBalance === null) {
    reasons.push(
      reason(
