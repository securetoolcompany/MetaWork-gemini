...
export function calculateProductRevenueAllocation({
  ...
  unitBaseCostCents,
  unitGrossIpLicenseCostCents,
}) {
  const normalizedQuantity = assertSafeInteger(quantity, 'quantity', {
    minimum: 1,
  });

  const normalizedUnitMerchandisePriceCents = assertSafeInteger(
    unitMerchandisePriceCents,
    'unitMerchandisePriceCents',
    { minimum: 0 }
  );

  const normalizedUnitBaseCostCents = assertSafeInteger(
    unitBaseCostCents,
    'unitBaseCostCents',
    { minimum: 0 }
  );

  const normalizedUnitGrossIpLicenseCostCents = assertSafeInteger(
    unitGrossIpLicenseCostCents,
    'unitGrossIpLicenseCostCents',
    { minimum: 0 }
  );

  const unitCreatorRevenueCents =
    normalizedUnitMerchandisePriceCents -
    normalizedUnitBaseCostCents -
    normalizedUnitGrossIpLicenseCostCents;

  if (unitCreatorRevenueCents < 0) {
    throw new Error(
      'unitMerchandisePriceCents must cover base cost and gross IP license cost'
    );
  }

  const merchandiseGrossCents =
    normalizedQuantity * normalizedUnitMerchandisePriceCents;

  const baseCostCents =
    normalizedQuantity * normalizedUnitBaseCostCents;

  const grossIpLicenseCostCents =
    normalizedQuantity * normalizedUnitGrossIpLicenseCostCents;

  const allocationCents =
    normalizedQuantity * unitCreatorRevenueCents;

  const usdcAtomicUnits =
    allocationCents * USDC_ATOMIC_UNITS_PER_USD_CENT;

  assertSafeInteger(merchandiseGrossCents, 'merchandiseGrossCents');
  assertSafeInteger(baseCostCents, 'baseCostCents');
  assertSafeInteger(
    grossIpLicenseCostCents,
    'grossIpLicenseCostCents'
  );
  assertSafeInteger(
    unitCreatorRevenueCents,
    'unitCreatorRevenueCents'
  );
  assertSafeInteger(allocationCents, 'allocationCents');
  assertSafeInteger(usdcAtomicUnits, 'usdcAtomicUnits');
  ...
  return Object.freeze({
    ...
    quantity: normalizedQuantity,
    unitMerchandisePriceCents: normalizedUnitMerchandisePriceCents,
    unitBaseCostCents: normalizedUnitBaseCostCents,
    unitGrossIpLicenseCostCents:
      normalizedUnitGrossIpLicenseCostCents,
    unitCreatorRevenueCents,
