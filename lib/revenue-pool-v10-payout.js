...
/**
 * Builds the unsigned V10 create_payout_round transaction group.
 *
 * Group order:
 * 0. Application call:
 *    args = [
 ...
 *      poolKey UTF-8 bytes,
 *      total USDC uint64,
 *      first packed payee chunk,
 *      ...additional chunks,
 *      companion payment index uint64,
 *    ]
 * 1. Companion ALGO payment to the app address for exact round-box MBR.
 *
 * This function does not sign, submit, or broadcast.
 */
export function buildUnsignedV10CreatePayoutRoundGroup({
  ...
  const payeeChunks = splitPayeeChunks(packedPayees);
	  ...
	  const applicationArgCount =
			3 + payeeChunks.length + 1;
