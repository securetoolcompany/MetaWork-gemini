...
function assertMatchingImmutableBatch(existingBatch, expectedBatch) {
  ...
  for (const fieldName of immutableFields) {
    if (existingBatch[fieldName] !== expectedBatch[fieldName]) {
      throw new Error(
        `[revenue-settlement-batches] Existing batch immutable field mismatch: ${fieldName}`
      );
    }
  }

  const existingRowIds = (existingBatch.ledgerRowIds ?? [])
    .map((rowId) => String(rowId))
    .sort();

  const expectedRowIds = (expectedBatch.ledgerRowIds ?? [])
    .map((rowId) => String(rowId))
    .sort();

  const existingIdempotencyKeys = (
    existingBatch.ledgerIdempotencyKeys ?? []
  )
    .map((idempotencyKey) => String(idempotencyKey))
    .sort();

  const expectedIdempotencyKeys = (
    expectedBatch.ledgerIdempotencyKeys ?? []
  )
    .map((idempotencyKey) => String(idempotencyKey))
    .sort();

  if (
    JSON.stringify(existingRowIds) !== JSON.stringify(expectedRowIds) ||
    JSON.stringify(existingIdempotencyKeys) !==
      JSON.stringify(expectedIdempotencyKeys)
  ) {
    throw new Error(
      '[revenue-settlement-batches] Existing batch immutable ledger-row mismatch'
    );
  }

  if (
    !REVENUE_SETTLEMENT_BATCH_STATUSES.includes(existingBatch.status)
  ) {
    throw new Error(
      `[revenue-settlement-batches] Existing batch has invalid status: ${String(
...
export async function createSettlementBatchFromEligibleRows({
  ...
  assertMatchingImmutableBatch(batch, batchDocument);
