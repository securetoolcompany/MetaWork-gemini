...
function createFreshnessSnapshot(overrides = {}) {
  return {
    revenuePoolAppId: 7001,
    usdcAssetId: 10458941,
    adminAddress: 'ADMINADDRESS',
    ipAssetId: 'ip-asset-a',
    heldUsdcAtomicUnits: '1000000',
    currentRoundId: '0',
    stakeholderCount: 1,
    releaseStatus: 'ready',
    requiredMbrMicroAlgos: 40500,
    requiredFeesMicroAlgos: 3000,
    requiredTotalMicroAlgos: 43500,
    ...overrides,
  };
}
...
function createPreflightResult({
  ...
  groupId = GROUP_ID,
  reasons = [],
  freshnessSnapshot = createFreshnessSnapshot(),
} = {}) {
  return {
    ok,
    reasons,
...
const changedParamsAlgod = createAlgodClient();

const changedSuggestedParamsButSameState =
  await submitApprovedV7ReleaseHeldGroup({
    enabled: true,
    userConfirmedSubmission: true,
    signedResult: createSignedResult(),
    preflightResult: createPreflightResult(),
    ...
    algodClient: changedParamsAlgod,
    maxConfirmationAttempts: 1,
    confirmationPollIntervalMilliseconds: 0,
  });

assert.equal(changedSuggestedParamsButSameState.ok, true);
assert.equal(changedSuggestedParamsButSameState.outcome, 'confirmed');
assert.equal(changedParamsAlgod.submittedTransactions.length, 2);
