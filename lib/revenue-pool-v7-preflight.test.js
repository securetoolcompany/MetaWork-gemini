...
assert.deepEqual(successfulPreflight.freshnessSnapshot, {
  revenuePoolAppId: 7001,
  usdcAssetId: 10458941,
  adminAddress,
  ipAssetId: 'ip-asset-a',
  heldUsdcAtomicUnits: '8530000',
  currentRoundId: '4',
  stakeholderCount: 2,
  releaseStatus: 'ready',
  requiredMbrMicroAlgos: 51300,
  requiredFeesMicroAlgos: 3000,
  requiredTotalMicroAlgos: 54300,
});
...
const refreshOnlyPreflight = preflightV7ReleaseHeld(
  createPreflightInput({
    suggestedParams: {
      ...suggestedParams,
      firstValid: 999,
...
assert.equal(refreshOnlyPreflight.ok, true);
assert.equal(refreshOnlyPreflight.unsignedGroup, null);
assert.equal(refreshOnlyPreflight.proposedGroup, null);
assert.deepEqual(
  refreshOnlyPreflight.freshnessSnapshot,
  successfulPreflight.freshnessSnapshot
);
