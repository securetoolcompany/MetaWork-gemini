...
function assertSignedResult(signedResult) {
  ...
  if (typeof signedResult.groupId !== 'string' || !signedResult.groupId) {
...
function assertFreshnessSnapshot(value) {
  if (!value || typeof value !== 'object') {
    return 'A release freshness snapshot is required.';
  }
  ...
  for (const field of FRESHNESS_FIELDS) {
    if (
      ...
      return `Release freshness snapshot is missing ${field}.`;
    }
  }
...
function getFreshnessDifferences(previous, fresh) {
  return FRESHNESS_FIELDS.flatMap((field) => {
    const previousValue = String(previous[field]);
    const freshValue = String(fresh[field]);

    if (previousValue === freshValue) {
      return [];
    }

    return [
      `Freshness mismatch for ${field}: approved ${previousValue}, live ${freshValue}.`,
    ];
  });
}
...
export async function submitApprovedV7ReleaseHeldGroup({
  ...
  const initialSnapshotError = assertFreshnessSnapshot(
    preflightResult.freshnessSnapshot
  );
  ...
  const freshSnapshotError = assertFreshnessSnapshot(
    freshPreflight.freshnessSnapshot
  );
  ...
  if (freshSnapshotError) {
    return outcome({
      ...
      outcome: 'stale_preflight',
      groupId: signedResult.groupId,
      reasons: [freshSnapshotError],
    });
  }

  const freshnessDifferences = getFreshnessDifferences(
    preflightResult.freshnessSnapshot,
    freshPreflight.freshnessSnapshot
  );

  if (freshnessDifferences.length > 0) {
    return outcome({
      ok: false,
      outcome: 'stale_preflight',
      groupId: signedResult.groupId,
      reasons: freshnessDifferences,
    });
  }
