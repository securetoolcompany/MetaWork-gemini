...
export async function linkWalletToUser(userId, walletAddress, chain = 'algorand') {
  ...
  const currentWallets = Array.isArray(user.wallets) ? user.wallets : [];
  ...
  for (const wallet of currentWallets) {
    ...
    const identity = `${wallet.chain || 'algorand'}:${address}`;

    // Keeps one copy of any pre-existing duplicate.
  ...
  const walletIdentity = `${chain}:${normalizedAddress}`;
