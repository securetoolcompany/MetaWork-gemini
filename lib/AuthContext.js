...
export function AuthProvider({ children }) {
  ...
  const { accountAddress, isWalletReady } = useWallet(); // Added isWalletReady
  ...
  // 🛡️ THE STRICT AUTH WATCHDOG
  useEffect(() => {
    // Don't enforce rules while either system is still booting up (prevents race conditions)
    if (loading || !isWalletReady) return;

    if (user) {
      // Rule 1: You must have a connected wallet to maintain an active session
      if (!accountAddress) {
        console.warn('Auth Watchdog: Wallet disconnected. Terminating stale session.');
        logout();
      }
      // Rule 2: Your active wallet MUST exactly match the wallet recorded in your secure cookie
      else if (user.walletAddress && user.walletAddress !== accountAddress) {
