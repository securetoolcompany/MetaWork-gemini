import assert from 'node:assert/strict';
import test from 'node:test';

import algosdk from 'algosdk';

import {
  V10_PAYOUT_ACTION,
  buildUnsignedV10CreatePayoutRoundGroup,
  rebuildV10CreatePayoutRoundGroupFromUnsignedTransactions,
  calculateV10PayoutRoundBoxMbrMicroalgos,
  createV10PoolBoxName,
  createV10RoundBoxName,
  createV10RoundPayeesBytes,
} from './revenue-pool-v10-payout.js';

function makeAddress() {
  return algosdk.generateAccount().addr.toString();
}

function makeSuggestedParams() {
  return {
    fee: 1000,
    minFee: 1000,
    flatFee: true,
    firstValid: 1,
    lastValid: 1001,
    genesisHash: new Uint8Array(32),
    genesisID: 'testnet-v1.0',
  };
}

function addressByteCompare(left, right) {
  return Buffer.compare(
    Buffer.from(algosdk.decodeAddress(left).publicKey),
    Buffer.from(algosdk.decodeAddress(right).publicKey),
  );
}

function decodeUnsignedTransaction(encodedTransaction) {
  return algosdk.decodeUnsignedTransaction(
    new Uint8Array(Buffer.from(encodedTransaction, 'base64')),
  );
}

test('builds the exact unsigned V10 create_payout_round group', () => {
  const sender = makeAddress();
  const first = makeAddress();
  const second = makeAddress();

  const group = buildUnsignedV10CreatePayoutRoundGroup({
    appId: 7001,
    poolKey: 'ip-a',
    sender,
    roundPayees: [
      {
        address: second,
        amountUsdcAtomicUnits: 300_000,
      },
      {
        address: first,
        amountUsdcAtomicUnits: 700_000,
      },
    ],
    totalUsdcAtomicUnits: 1_000_000,
    nextRoundId: 1,
    suggestedParams: makeSuggestedParams(),
  });

  assert.equal(group.action, V10_PAYOUT_ACTION);
  assert.equal(group.appId, 7001);
  assert.equal(group.poolKey, 'ip-a');
  assert.equal(group.sender, sender);
  assert.equal(group.totalUsdcAtomicUnits, 1_000_000);
  assert.equal(group.nextRoundId, 1);
  assert.equal(group.recipientCount, 2);
  assert.equal(group.transactionCount, 2);
	assert.equal(group.appCallTransactionIndex, 0);
	assert.equal(group.companionPaymentTransactionIndex, 1);
	assert.equal(group.companionPaymentIndex, 1);

  assert.deepEqual(
    group.roundPayees.map((payee) => payee.address),
    [first, second].sort(addressByteCompare),
  );

  assert.equal(
    group.roundPayees.reduce(
      (total, payee) => total + payee.amountUsdcAtomicUnits,
      0,
    ),
    1_000_000,
  );

  assert.equal(group.unsignedTransactionsBase64.length, 2);
  assert.match(group.groupId, /^[A-Za-z0-9+/]{43}=$/);
  assert.match(group.transactionIds.appCall, /^[A-Z2-7]{52}$/);
  assert.match(
    group.transactionIds.companionPayment,
    /^[A-Z2-7]{52}$/,
  );
  assert.match(group.unsignedTransactionHash, /^sha256:[a-f0-9]{64}$/);
  assert.match(group.roundPayeesHash, /^sha256:[a-f0-9]{64}$/);

  assert.equal(
    group.roundBoxMbrMicroalgos,
    calculateV10PayoutRoundBoxMbrMicroalgos({
      poolKey: 'ip-a',
      recipientCount: 2,
    }),
  );

  
	const appCall = decodeUnsignedTransaction(
    group.unsignedTransactionsBase64[0],
  );
	const companionPayment = decodeUnsignedTransaction(
    group.unsignedTransactionsBase64[1],
  );
  
  assert.equal(companionPayment.type, 'pay');

	assert.equal(
		companionPayment.payment.amount,
		BigInt(group.roundBoxMbrMicroalgos),
	);

	assert.equal(
		algosdk.encodeAddress(companionPayment.payment.receiver.publicKey),
		group.appAddress.toString(),
	);

  assert.equal(appCall.type, 'appl');
	assert.equal(group.companionPaymentIndex, 1);
});

test('rebuilds and validates the exact persisted V10 payout group', () => {
  const sender = makeAddress();
  const first = makeAddress();
  const second = makeAddress();

  const input = {
    appId: 7001,
    poolKey: 'ip-a',
    sender,
    roundPayees: [
      {
        address: second,
        amountUsdcAtomicUnits: 300_000,
      },
      {
        address: first,
        amountUsdcAtomicUnits: 700_000,
      },
    ],
    totalUsdcAtomicUnits: 1_000_000,
    nextRoundId: 1,
    suggestedParams: makeSuggestedParams(),
  };

  const built = buildUnsignedV10CreatePayoutRoundGroup(input);

  const rebuilt =
    rebuildV10CreatePayoutRoundGroupFromUnsignedTransactions({
      unsignedTransactionsBase64:
        built.unsignedTransactionsBase64,
      appId: input.appId,
      poolKey: input.poolKey,
      sender: input.sender,
      roundPayees: input.roundPayees,
      totalUsdcAtomicUnits: input.totalUsdcAtomicUnits,
      nextRoundId: input.nextRoundId,
    });

  assert.equal(rebuilt.action, V10_PAYOUT_ACTION);
  assert.equal(rebuilt.appId, built.appId);
  assert.equal(rebuilt.poolKey, built.poolKey);
  assert.equal(rebuilt.sender, built.sender);
  assert.equal(
    rebuilt.totalUsdcAtomicUnits,
    built.totalUsdcAtomicUnits,
  );
  assert.equal(rebuilt.nextRoundId, built.nextRoundId);
  assert.equal(rebuilt.recipientCount, built.recipientCount);
  assert.equal(
    rebuilt.roundBoxMbrMicroalgos,
    built.roundBoxMbrMicroalgos,
  );
  assert.equal(rebuilt.groupId, built.groupId);
  assert.equal(
    rebuilt.unsignedTransactionHash,
    built.unsignedTransactionHash,
  );
  assert.equal(
    rebuilt.roundPayeesHash,
    built.roundPayeesHash,
  );
  assert.deepEqual(
    rebuilt.transactionIds,
    built.transactionIds,
  );
  assert.deepEqual(
    rebuilt.unsignedTransactionsBase64,
    built.unsignedTransactionsBase64,
  );
});

test('rejects a tampered persisted V10 payout group', () => {
  const sender = makeAddress();
  const recipient = makeAddress();

  const input = {
    appId: 7001,
    poolKey: 'ip-a',
    sender,
    roundPayees: [
      {
        address: recipient,
        amountUsdcAtomicUnits: 1_000_000,
      },
    ],
    totalUsdcAtomicUnits: 1_000_000,
    nextRoundId: 1,
    suggestedParams: makeSuggestedParams(),
  };

  const built = buildUnsignedV10CreatePayoutRoundGroup(input);

  const tamperedUnsignedTransactionsBase64 = [
    `${built.unsignedTransactionsBase64[0].slice(0, -4)}AAAA`,
    built.unsignedTransactionsBase64[1],
  ];

  assert.throws(
    () =>
      rebuildV10CreatePayoutRoundGroupFromUnsignedTransactions({
        unsignedTransactionsBase64:
          tamperedUnsignedTransactionsBase64,
        appId: input.appId,
        poolKey: input.poolKey,
        sender: input.sender,
        roundPayees: input.roundPayees,
        totalUsdcAtomicUnits: input.totalUsdcAtomicUnits,
        nextRoundId: input.nextRoundId,
      }),
    /unsigned transactions|application call|group|transaction/i,
  );
});

test('packs canonical recipient entries as address bytes plus uint64 amount', () => {
  const first = makeAddress();
  const second = makeAddress();

  const packed = createV10RoundPayeesBytes({
    roundPayees: [
      {
        address: second,
        amountUsdcAtomicUnits: 300_000,
      },
      {
        address: first,
        amountUsdcAtomicUnits: 700_000,
      },
    ],
    totalUsdcAtomicUnits: 1_000_000,
  });

  const ordered = [first, second].sort(addressByteCompare);

  assert.equal(packed.length, 80);

  assert.deepEqual(
    Buffer.from(packed.subarray(0, 32)),
    Buffer.from(algosdk.decodeAddress(ordered[0]).publicKey),
  );
  assert.equal(
    Buffer.from(packed.subarray(32, 40)).readBigUInt64BE(),
    BigInt(
      ordered[0] === first
        ? 700_000
        : 300_000,
    ),
  );

  assert.deepEqual(
    Buffer.from(packed.subarray(40, 72)),
    Buffer.from(algosdk.decodeAddress(ordered[1]).publicKey),
  );
  assert.equal(
    Buffer.from(packed.subarray(72, 80)).readBigUInt64BE(),
    BigInt(
      ordered[1] === first
        ? 700_000
        : 300_000,
    ),
  );
});

test('uses exact V10 pool and round box names and payout-round MBR formula', () => {
  assert.deepEqual(
    Buffer.from(createV10PoolBoxName('ip-a')),
    Buffer.from('p_ip-a'),
  );

  assert.deepEqual(
    Buffer.from(createV10RoundBoxName('ip-a', 1)),
    Buffer.concat([
      Buffer.from('rnd_ip-a'),
      Buffer.from([0, 0, 0, 0, 0, 0, 0, 1]),
    ]),
  );

  assert.equal(
    calculateV10PayoutRoundBoxMbrMicroalgos({
      poolKey: 'ip-a',
      recipientCount: 1,
    }),
    32_500,
  );

  assert.equal(
    calculateV10PayoutRoundBoxMbrMicroalgos({
      poolKey: 'ip-a',
      recipientCount: 2,
    }),
    48_900,
  );
});

test('builds identical payout groups from identical frozen inputs', () => {
  const sender = makeAddress();
  const first = makeAddress();
  const second = makeAddress();

  const input = Object.freeze({
    appId: 7001,
    poolKey: 'ip-a',
    sender,
    roundPayees: Object.freeze([
      Object.freeze({
        address: second,
        amountUsdcAtomicUnits: 300_000,
      }),
      Object.freeze({
        address: first,
        amountUsdcAtomicUnits: 700_000,
      }),
    ]),
    totalUsdcAtomicUnits: 1_000_000,
    nextRoundId: 1,
    suggestedParams: Object.freeze(makeSuggestedParams()),
  });

  const firstGroup =
    buildUnsignedV10CreatePayoutRoundGroup(input);
  const secondGroup =
    buildUnsignedV10CreatePayoutRoundGroup(input);

  assert.equal(firstGroup.groupId, secondGroup.groupId);
  assert.deepEqual(
    firstGroup.transactionIds,
    secondGroup.transactionIds,
  );
  assert.deepEqual(
    firstGroup.unsignedTransactionsBase64,
    secondGroup.unsignedTransactionsBase64,
  );
  assert.equal(
    firstGroup.unsignedTransactionHash,
    secondGroup.unsignedTransactionHash,
  );
  assert.equal(
    firstGroup.roundPayeesHash,
    secondGroup.roundPayeesHash,
  );
});

test('rejects invalid payout inputs and non-reconciling recipients', () => {
  const sender = makeAddress();
  const recipient = makeAddress();

  assert.throws(
    () =>
      buildUnsignedV10CreatePayoutRoundGroup({
        appId: 0,
        poolKey: 'ip-a',
        sender,
        roundPayees: [
          {
            address: recipient,
            amountUsdcAtomicUnits: 1_000_000,
          },
        ],
        totalUsdcAtomicUnits: 1_000_000,
        nextRoundId: 1,
        suggestedParams: makeSuggestedParams(),
      }),
    /appId must be a safe integer/,
  );

  assert.throws(
    () =>
      buildUnsignedV10CreatePayoutRoundGroup({
        appId: 7001,
        poolKey: 'ip-a',
        sender,
        roundPayees: [
          {
            address: recipient,
            amountUsdcAtomicUnits: 1_000_000,
          },
        ],
        totalUsdcAtomicUnits: 1_000_000,
        nextRoundId: 0,
        suggestedParams: makeSuggestedParams(),
      }),
    /nextRoundId must be a safe integer/,
  );

  assert.throws(
    () =>
      buildUnsignedV10CreatePayoutRoundGroup({
        appId: 7001,
        poolKey: 'ip-a',
        sender,
        roundPayees: [
          {
            address: recipient,
            amountUsdcAtomicUnits: 999_999,
          },
        ],
        totalUsdcAtomicUnits: 1_000_000,
        nextRoundId: 1,
        suggestedParams: makeSuggestedParams(),
      }),
    /do not reconcile/,
  );

  assert.throws(
    () =>
      createV10RoundPayeesBytes({
        roundPayees: [
          {
            address: recipient,
            amountUsdcAtomicUnits: 500_000,
          },
          {
            address: recipient,
            amountUsdcAtomicUnits: 500_000,
          },
        ],
        totalUsdcAtomicUnits: 1_000_000,
      }),
    /duplicate recipient address/,
  );
});