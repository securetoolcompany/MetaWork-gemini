...
function makeClaimedPoolBox(stakeholderAddress, revUnits = 10_000) {
  ...
  Buffer.from(
    algosdk.decodeAddress(stakeholderAddress).publicKey,
  ).copy(bytes, 73);

  bytes.writeUInt16BE(revUnits, 105);
  bytes.writeUInt8(1, 107);
    ...
    collection(name) {
      ...
      return {
        ...
        async findOneAndUpdate(filter, update) {
            ...
            if (
              filter.status === 'deposit_prepared' &&
              filter['depositAttempt.status'] === 'prepared' &&
              filter['depositAttempt.groupId'] &&
              filter['depositAttempt.unsignedTransactionHash'] &&
              filter['depositAttempt.transactionIds.usdcTransfer'] &&
              filter['depositAttempt.transactionIds.appCall']
            ) {
              state.submittedDepositUpdateCalls += 1;

              const filterMatches =
                state.batch._id === filter._id &&
                state.batch.status === 'deposit_prepared' &&
                ...
                state.batch.depositAttempt?.status ===
                  filter['depositAttempt.status'] &&
                state.batch.depositAttempt?.groupId ===
                  filter['depositAttempt.groupId'] &&
                state.batch.depositAttempt?.unsignedTransactionHash ===
                  filter['depositAttempt.unsignedTransactionHash'] &&
                state.batch.depositAttempt?.transactionIds?.usdcTransfer ===
                  filter['depositAttempt.transactionIds.usdcTransfer'] &&
              ...
              state.batch.depositAttempt = {
                ...
                submittedAt: clonedSet['depositAttempt.submittedAt'],
              };
            ...
            if (
                ...
                state.batch.confirmedRound =
                    clonedSet.confirmedRound;
                state.batch.updatedAt = clonedSet.updatedAt;
...
function makeSnapshotAlgodClient({
  poolBox = makeClaimedPoolBox(makeAddress()),
} = {}) {
  let calls = 0;
...
test('persists a recipient snapshot once and leaves ledger state outside this unit', async () => {
  ...
  assert.equal(algodClient.calls, 2);
...
function makeUnsignedUsdcDepositPreflight(batch) {
  const depositor = makeAddress();
  ...
  const unsignedGroup = buildV10DepositUsdcGroup({
    revenuePoolAppId: batch.revenuePoolAppId,
    usdcAssetId: 10458941,
    depositorAddress: depositor,
    poolKey: batch.poolKey,
  ...
  return {
    ...
    proposedGroup: {
      action: 'deposit_usdc',
      transactionCount: unsignedGroup.transactionCount ?? 2,
      usdcTransferTransactionIndex: 0,
      appCallTransactionIndex: 1,
      groupId: unsignedGroup.groupId,
      unsignedTransactionHash: unsignedGroup.unsignedTransactionHash,
...
test('forwards unallocated USDC deposit type through live preflight', async () => {
  const batch = makeRecipientSnapshotPreparedBatch();
  const db = createPersistenceDb({ batch });
  let receivedInput = null;

  const result = await prepareAndPersistV10DepositAttempt({
    db,
    batchId: batch._id,
    depositType: 'usdc',
    preflightLive: async (input) => {
      receivedInput = input;

      return makeUnsignedUsdcDepositPreflight(input.batch);
    },
  });

  assert.equal(receivedInput.depositType, 'usdc');
  assert.equal(db.state.batch.depositAttempt.depositType, 'usdc');
  assert.equal(
    db.state.batch.depositAttempt.operation,
    'v10_usdc_deposit',
  );
  });

test('rejects unsupported V10 deposit types before loading a batch', async () => {
  ...
  await assert.rejects(
    () =>
      prepareAndPersistV10DepositAttempt({
        db,
        batchId: 'batch-1',
        depositType: 'unallocated',
      }),
    /depositType must be "held" or "usdc"/,
  );

  assert.equal(collectionCalls, 0);
});

test('persists submitted V10 deposit facts without replacing the prepared group', async () => {
  const { db } =
    await createDepositPreparedBatchForConfirmation();

  const originalAttempt = clone(db.state.batch.depositAttempt);
  const submittedAt = new Date('2026-08-18T16:10:30.000Z');

  const result = await persistSubmittedV10DepositAttempt({
    db,
    batchId: 'batch-1',
    submittedAt,
  });

  assert.equal(result.status, 'deposit_submitted');
  assert.equal(result.depositAttempt.status, 'submitted');
  assert.equal(
    result.depositAttempt.submittedAt.toISOString(),
    submittedAt.toISOString(),
  );
  assert.equal(result.depositAttempt.confirmedAt, null);

  assert.equal(
    result.depositAttempt.groupId,
    originalAttempt.groupId,
  );
  assert.equal(
    result.depositAttempt.unsignedTransactionHash,
    originalAttempt.unsignedTransactionHash,
  );
  assert.deepEqual(
    result.depositAttempt.unsignedTransactionsBase64,
    originalAttempt.unsignedTransactionsBase64,
  );
  assert.deepEqual(
    result.depositAttempt.transactionIds,
    originalAttempt.transactionIds,
  );
  assert.deepEqual(
    result.depositAttempt.target,
    originalAttempt.target,
  );
  assert.equal(
    result.depositAttempt.amountUsdcAtomicUnits,
    originalAttempt.amountUsdcAtomicUnits,
  );
  assert.equal(
    result.depositAttempt.usdcTransferTransactionIndex,
    0,
  );
  assert.equal(
    result.depositAttempt.appCallTransactionIndex,
    1,
  );

  assert.equal(db.state.batch.status, 'deposit_submitted');
  assert.equal(
    db.state.batch.depositAttempt.status,
    'submitted',
  );
  assert.equal(
    db.state.batch.depositAttempt.submittedAt.toISOString(),
    submittedAt.toISOString(),
  );
  assert.equal(db.state.submittedDepositUpdateCalls, 1);
});

test('returns the existing submitted V10 deposit attempt idempotently', async () => {
  const { db } =
    await createDepositPreparedBatchForConfirmation();

  const firstSubmittedAt = new Date(
    '2026-08-18T16:10:30.000Z',
  );

  const first = await persistSubmittedV10DepositAttempt({
    db,
    batchId: 'batch-1',
    submittedAt: firstSubmittedAt,
  });

  const retry = await persistSubmittedV10DepositAttempt({
    db,
    batchId: 'batch-1',
    submittedAt: new Date('2026-08-18T16:11:00.000Z'),
  });

  assert.equal(first.status, 'deposit_submitted');
  assert.equal(retry.status, 'deposit_submitted');
  assert.equal(
    retry.depositAttempt.submittedAt.toISOString(),
    firstSubmittedAt.toISOString(),
  );
  assert.equal(
    retry.depositAttempt.groupId,
    first.depositAttempt.groupId,
  );
  assert.equal(
    retry.depositAttempt.transactionIds.appCall,
    first.depositAttempt.transactionIds.appCall,
  );
  assert.equal(db.state.submittedDepositUpdateCalls, 1);
});

test('confirms a submitted V10 deposit using the durable app-call transaction ID', async () => {
  const { db } =
    await createDepositPreparedBatchForConfirmation();

  await persistSubmittedV10DepositAttempt({
    db,
    batchId: 'batch-1',
    submittedAt: new Date('2026-08-18T16:10:30.000Z'),
  });
  ...
  assert.equal(result.depositAttempt.status, 'confirmed');
...
test('persists confirmed V10 deposit facts on the batch only', async () => {
  ...
  assert.equal(db.state.batch.confirmedRound, '123456');
      ...
      if (name === 'revenue_settlement_batches') {
        return {
          ...
          async findOneAndUpdate(filter, update) {
            ...
            if (
              filter._id === state.batch._id &&
              filter.status === 'round_created' &&
              filter.payoutSubmissionAttempt === null &&
              filter.revenueRoundId === state.batch.revenueRoundId &&
              filter.roundPayeesHash === state.batch.roundPayeesHash
            ) {
              state.batch = {
                ...state.batch,
            ...
            if (
              filter._id === state.batch._id &&
              filter.status === 'payout_prepared' &&
              state.batch.status === 'payout_prepared' &&
              filter.revenueRoundId === state.batch.revenueRoundId &&
              filter.revenueRoundTxId === null &&
              filter['payoutSubmissionAttempt.operation'] ===
                state.batch.payoutSubmissionAttempt?.operation &&
              filter['payoutSubmissionAttempt.status'] ===
                state.batch.payoutSubmissionAttempt?.status &&
              filter['payoutSubmissionAttempt.groupId'] ===
                state.batch.payoutSubmissionAttempt?.groupId &&
              filter['payoutSubmissionAttempt.unsignedTransactionHash'] ===
                state.batch.payoutSubmissionAttempt
                  ?.unsignedTransactionHash &&
              filter[
                'payoutSubmissionAttempt.transactionIds.appCall'
              ] ===
                state.batch.payoutSubmissionAttempt?.transactionIds
                  ?.appCall &&
              filter[
                'payoutSubmissionAttempt.transactionIds.companionPayment'
              ] ===
                state.batch.payoutSubmissionAttempt?.transactionIds
                  ?.companionPayment &&
              filter[
                ...
                state.batch.payoutSubmissionAttempt
                  ?.totalUsdcAtomicUnits &&
              filter['payoutSubmissionAttempt.roundPayeesHash'] ===
                state.batch.payoutSubmissionAttempt?.roundPayeesHash &&
              filter['payoutSubmissionAttempt.nextRoundId'] ===
                state.batch.payoutSubmissionAttempt?.nextRoundId &&
              filter['payoutSubmissionAttempt.sender'] ===
                state.batch.payoutSubmissionAttempt?.sender
            ) {
              state.batch = {
                ...state.batch,
                ...update.$set,
                payoutSubmissionAttempt: {
                  ...state.batch.payoutSubmissionAttempt,
                  ...update.$set.payoutSubmissionAttempt,
                  status:
                    update.$set['payoutSubmissionAttempt.status'] ??
                    state.batch.payoutSubmissionAttempt.status,
                  submittedAt:
                    update.$set['payoutSubmissionAttempt.submittedAt'] ??
                    state.batch.payoutSubmissionAttempt.submittedAt,
                },
              };
...
test('prepares one durable V10 create_payout_round submission attempt', async () => {
  const { batch, materialization } =
    await createDepositedBatchAndMaterializationForRound();
  ...
  const poolBox = Buffer.alloc(108);
  poolBox.writeBigUInt64BE(
    BigInt(batch.revenueTokenAssetId),
    0,
  );
  poolBox.writeBigUInt64BE(0n, 8);
  poolBox.writeBigUInt64BE(0n, 24);
  poolBox.writeBigUInt64BE(0n, 32);
  poolBox[40] = 1;

  const algodClient = {
    getApplicationBoxByName(appId, boxName) {
      assert.equal(appId, batch.revenuePoolAppId);
      assert.deepEqual(
        Buffer.from(boxName),
        Buffer.from(`p_${batch.poolKey}`),
      );

      return {
        do: async () => ({
          value: new Uint8Array(poolBox),
        }),
      };
    },
  };
  ...
  const result = await prepareCreatePayoutRoundSubmission({
    db,
    batchId: 'batch-1',
    sender,
    algodClient,
  ...
  assert.equal(
    result.attemptKey,
    'batch-1:create-payout-round:v1',
  );
  assert.equal(result.operation, 'v10_create_payout_round');
  assert.equal(
    result.payoutRoundKey,
    'batch-1:payout-round:v1',
  );
  ...
  assert.equal(result.nextRoundId, 1);
  assert.equal(result.unsignedTransactionsBase64.length, 2);
  assert.equal(result.transactionIds.appCall.length, 52);
  assert.equal(
    result.transactionIds.companionPayment.length,
    52,
  );
  assert.equal(result.submittedAt, null);
  assert.equal(result.confirmedAt, null);

  assert.equal(db.state.batch.status, 'payout_prepared');
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.attemptKey,
    result.attemptKey,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.nextRoundId,
    1,
  );
  assert.equal(db.state.batch.revenueRoundTxId, null);
});

test('persists submitted V10 payout-round facts without replacing the prepared group', async () => {
  const { batch, materialization } =
    await createDepositedBatchAndMaterializationForRound();
  ...
  const beforeAttempt = clone(
    db.state.batch.payoutSubmissionAttempt,
  );

  const submittedAt = new Date('2026-08-18T16:15:00.000Z');
  ...
  assert.equal(submitted.status, 'payout_submitted');
  assert.equal(
    submitted.revenueRoundTxId,
    beforeAttempt.transactionIds.appCall,
  );

  assert.equal(
    submitted.payoutSubmissionAttempt.status,
    'submitted',
  );
  assert.equal(
    submitted.payoutSubmissionAttempt.submittedAt.getTime(),
    submittedAt.getTime(),
  );
  assert.equal(
    submitted.payoutSubmissionAttempt.confirmedAt,
    null,
  );

  assert.equal(db.state.batch.status, 'payout_submitted');
  assert.equal(
    db.state.batch.revenueRoundTxId,
    beforeAttempt.transactionIds.appCall,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.status,
    'submitted',
  );
  assert.equal(
    new Date(
      db.state.batch.payoutSubmissionAttempt.submittedAt
    ).getTime(),
    submittedAt.getTime(),
  );
  ...
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.attemptKey,
    beforeAttempt.attemptKey,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.groupId,
    beforeAttempt.groupId,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt
      .unsignedTransactionHash,
    beforeAttempt.unsignedTransactionHash,
  );
  assert.deepEqual(
    db.state.batch.payoutSubmissionAttempt
      .unsignedTransactionsBase64,
    beforeAttempt.unsignedTransactionsBase64,
  );
  assert.deepEqual(
    db.state.batch.payoutSubmissionAttempt.transactionIds,
    beforeAttempt.transactionIds,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.payoutRoundKey,
    beforeAttempt.payoutRoundKey,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.totalUsdcAtomicUnits,
    beforeAttempt.totalUsdcAtomicUnits,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.roundPayeesHash,
    beforeAttempt.roundPayeesHash,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.nextRoundId,
    beforeAttempt.nextRoundId,
  );
  assert.equal(
    db.state.batch.payoutSubmissionAttempt.sender,
    beforeAttempt.sender,
  );
});

test('returns the existing submitted V10 payout round idempotently', async () => {
  const { batch, materialization } =
    await createDepositedBatchAndMaterializationForRound();
  ...
  await prepareCreatePayoutRoundSubmission({
    db,
    batchId: 'batch-1',
    sender,
    algodClient,
  ...
  const batchUpdateCallsAfterFirst = db.state.batchUpdateCalls;
  const attemptAfterFirst = clone(
    db.state.batch.payoutSubmissionAttempt,
  );
  ...
  const retry =
    await persistSubmittedV10CreatePayoutRoundSubmission({
      db,
      batchId: 'batch-1',
      submittedAt: new Date('2026-08-18T16:16:00.000Z'),
    });

  assert.equal(first.status, 'payout_submitted');
  assert.equal(retry.status, 'payout_submitted');
  assert.equal(
    retry.revenueRoundTxId,
    first.revenueRoundTxId,
  );
  assert.equal(
    retry.payoutSubmissionAttempt.status,
    'submitted',
  );
  assert.equal(
    retry.payoutSubmissionAttempt.submittedAt.getTime(),
    submittedAt.getTime(),
  );

  assert.equal(
    db.state.batchUpdateCalls,
    batchUpdateCallsAfterFirst,
  );
  assert.deepEqual(
    db.state.batch.payoutSubmissionAttempt,
    attemptAfterFirst,
...
test('rejects a tampered prepared payout app-call transaction ID before submission', async () => {
  const { batch, materialization } =
    await createDepositedBatchAndMaterializationForRound();
  ...
  const batchUpdateCallsBeforeTamper =
    db.state.batchUpdateCalls;

  db.state.batch.payoutSubmissionAttempt.transactionIds.appCall =
    'A'.repeat(52);
  ...
  await assert.rejects(
    () =>
      persistSubmittedV10CreatePayoutRoundSubmission({
        db,
        batchId: 'batch-1',
        submittedAt: new Date('2026-08-18T16:15:00.000Z'),
      }),
    (error) =>
      error instanceof RevenueSettlementRecipientSnapshotValidationError &&
      (
        error.code === 'PAYOUT_SUBMISSION_GROUP_MISMATCH' ||
        error.code === 'INVALID_PAYOUT_SUBMISSION_UNSIGNED_GROUP'
      ),
  );
