...
test('preflights an authorized V10 unallocated USDC deposit and proposes the exact unsigned group', () => {
  const result = preflightV10DepositUsdc(
    makeInput(),
  );
  ...
  assert.equal(
    result.appUsdcBalanceAtomicUnits,
    0,
  );
  ...
  assert.equal(
    result.unsignedGroup.transactions.length,
    2,
  );
...
test('rejects a depositor without USDC opt-in or sufficient frozen deposit amount', () => {
  ...
  assert.equal(noOptIn.unsignedGroup, null);
  ...
  assert.equal(insufficientUsdc.unsignedGroup, null);
});
...
test('rejects malformed pool boxes and invalid suggested params', () => {
  ...
  assert.equal(malformedPool.unsignedGroup, null);
  ...
  assert.equal(invalidParams.unsignedGroup, null);
});

test('accepts a valid 143-byte two-stakeholder V10 pool box', () => {
  const result = preflightV10DepositUsdc(
    makeInput({
      poolBoxValue: encodePoolBox({
        stakeholderCount: 2,
  ...
  assert.equal(result.ok, true);
  assert.equal(
    result.freshnessSnapshot.poolStakeholderCount,
    2,
  );
...
test('rejects malformed app USDC holding data without creating a group', () => {
  const result = preflightV10DepositUsdc(
    makeInput({
      appAccount: {
        amount: 1_000_000n,
  ...
  assert.equal(
    result.reasons.some(
      ({ code }) => code === 'APP_USDC_HOLDING_INVALID',
    ),
    true,
  );
  assert.equal(result.unsignedGroup, null);
