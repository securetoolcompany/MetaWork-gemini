...
export async function ensurePrintfulSyncProduct(productDoc, options = {}) {
  ...
  if (variantIds.length === 0) {
    ...
    if (!hasEdmTemplateId && files.length === 0) {
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        const existingSyncVariants = existing?.sync_variants || [];

        const existingMockupUrl =
          existingSyncProduct?.thumbnail_url ||
          existingSyncProduct?.thumbnail ||
          existingSyncProduct?.preview_image ||
          null;

        const variantMappings = buildVariantMappingsFromSyncVariants(existingSyncVariants);
