...
export async function ensurePrintfulSyncProduct(productDoc, options = {}) {
  ...
  const allowedVariantIds = new Set(
    (templateData?.result?.available_variant_ids || []).map(Number)
  );

  if (!allowedVariantIds.size) {
    throw new Error(
      `[printful-sync-product] Printful template ${productDoc.printfulTemplateId} has no available variant IDs`
    );
  }
  ...
  const sourceVariants = (
    ...
    return (
      ...
      catalogVariantId > 0 &&
      allowedVariantIds.has(catalogVariantId)
    );
  });
  ...
  if (variantIds.length !== sourceVariants.length) {
    throw new Error(
      "[printful-sync-product] One or more selected variants lack a valid catalog variant ID"
    );
  }
  ...
  if (variantIds.length === 0) {
        throw new Error(
      `[printful-sync-product] No in-stock catalog variants from product are allowed by Printful template ${productDoc.printfulTemplateId}`
    );
  }
