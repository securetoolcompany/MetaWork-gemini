...
export async function readV10DepositRecoveryState({
  ...
  try {
    ...
    if (getNotFoundStatus(cause) === 404) {
      if (
        !indexerClient ||
        typeof indexerClient.lookupTransactionByID !== 'function'
      ) {
        return Object.freeze({
          ...
          poolError: null,
          reason:
            'Stored app-call transaction is not currently available from Algod and no Indexer fallback is configured.',
        });
      }

      try {
        const indexerResult = await indexerClient
          .lookupTransactionByID(appCallTransactionId)
          .do();

        const indexedTransaction =
          indexerResult?.transaction ?? null;
        ...
        if (!indexedTransaction) {
          return Object.freeze({
            ...
            poolError: null,
            reason:
              'Stored app-call transaction is not currently available from Algod or Indexer.',
          });
        }

        const poolError = getPoolError(indexedTransaction);
        ...
        const confirmedRound =
          getConfirmedRound(indexedTransaction);
        ...
        return Object.freeze({
          ...
          poolError: null,
          reason:
            'Stored app-call transaction was found by Indexer but has no confirmed round.',
        });
      } catch (indexerCause) {
        return Object.freeze({
          ...
          poolError: null,
          reason:
            indexerCause?.message ||
            'Unable to read stored V10 deposit transaction from Indexer.',
        });
      }
    }
