...
import {
  ...
  rebuildV10CreatePayoutRoundGroupFromUnsignedTransactions,
...
function assertAlgodBoxReader(algodClient) {
  if (
    !algodClient ||
    typeof algodClient.getApplicationBoxByName !== 'function'
  ) {
    throw new TypeError(
      'algodClient must provide getApplicationBoxByName',
    );
  }
...
function decodeV10CurrentRoundId({
  boxValue,
}) {
  const bytes =
    boxValue instanceof Uint8Array
      ? new Uint8Array(boxValue)
      : Buffer.isBuffer(boxValue)
        ? new Uint8Array(boxValue)
  ...
  return view.getBigUint64(32, false);
}

async function readV10NextPayoutRoundId({
  algodClient,
  appId,
  poolKey,
}) {
  assertAlgodBoxReader(algodClient);
  ...
  try {
    poolBoxResponse = await algodClient
    ...
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Unable to read the V10 pool box before payout-round preparation',
      {
        code: 'V10_PAYOUT_POOL_BOX_READ_FAILED',
        cause,
      },
    );
  }

  const currentRoundId = decodeV10CurrentRoundId({
    boxValue: poolBoxResponse?.value,
  });

  if (currentRoundId > MAX_SAFE_BIGINT - 1n) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'V10 currentRoundId cannot be represented safely for payout-round preparation',
      { code: 'UNSAFE_V10_CURRENT_ROUND_ID' },
    );
  }

  return Object.freeze({
    currentRoundId: currentRoundId.toString(),
    nextRoundId: Number(currentRoundId + 1n),
  });
}
...
export function prepareV10RecipientSnapshot({
  ...
  const storedHolderSnapshot = {
    source: holderSnapshot.source ?? 'v10_effective_revenue_owners_v1',
    snapshotVersion: holderSnapshot.snapshotVersion ?? 'v2',
    appAddress: holderSnapshot.appAddress ?? null,
    ...
    poolBoxHash: holderSnapshot.poolBoxHash ?? null,
...
export async function prepareAndPersistV10RecipientSnapshot({
  ...
  const holderSnapshot = await snapshotV10EffectiveRevenueOwners({
    indexerClient,
    algodClient,
    appId: batch.revenuePoolAppId,
    poolKey: batch.poolKey,
    revenueTokenAssetId: batch.revenueTokenAssetId,
  });
...
function assertPreparedDepositAttempt(batch) {
  ...
  const depositType = attempt.depositType ?? 'held';

  if (depositType !== 'held' && depositType !== 'usdc') {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'depositAttempt.depositType must be "held" or "usdc"',
      { code: 'INVALID_DEPOSIT_ATTEMPT_TYPE' },
    );
  }
...
function createPreparedUsdcDepositAttempt({
  batch,
  livePreflight,
  preparedAt,
}) {
  ...
  if (
    unsignedGroup.target.revenuePoolAppId !== batch.revenuePoolAppId ||
    unsignedGroup.target.poolKey !== batch.poolKey ||
    unsignedGroup.target.usdcAssetId !== V10_DEPOSIT_USDC_ASSET_ID ||
    unsignedGroup.amountUsdcAtomicUnits !== batch.totalUsdcAtomicUnits
  ) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Live V10 deposit group does not match immutable batch target or amount',
      { code: 'LIVE_DEPOSIT_GROUP_BATCH_MISMATCH' },
  ...
  if (
    proposedGroup.groupId !== unsignedGroup.groupId ||
    proposedGroup.unsignedTransactionHash !== unsignedGroup.unsignedTransactionHash ||
    proposedGroup.transactionIds.usdcTransfer !== unsignedGroup.transactionIds.usdcTransfer ||
    proposedGroup.transactionIds.appCall !== unsignedGroup.transactionIds.appCall
  ) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Live V10 deposit preflight metadata does not match its unsigned group',
      { code: 'LIVE_DEPOSIT_GROUP_METADATA_MISMATCH' },
  ...
  return {
    attemptKey: `${String(batch._id)}:deposit:${V10_DEPOSIT_ATTEMPT_VERSION}`,
    operation: V10_DEPOSIT_OPERATION,
    depositType: 'usdc',
    status: 'prepared',
    groupId: unsignedGroup.groupId,
    unsignedTransactionHash: unsignedGroup.unsignedTransactionHash,
    unsignedTransactionsBase64: unsignedGroup.unsignedTransactionsBase64,
    transactionIds: {
    ...
    amountUsdcAtomicUnits: String(batch.totalUsdcAtomicUnits),
    preparedAt,
    submittedAt: null,
    confirmedAt: null,
    failureCode: null,
...
export async function prepareAndPersistV10DepositAttempt({
  ...
  if (typeof resolvedPreflightLive !== 'function') {
  ...
  if (
    ...
    const livePreflight = await resolvedPreflightLive({
    batch,
    ...preflightOptions,
    depositType,
  });
  ...
  if (!livePreflight || livePreflight.ok !== true) {
    ...
    const depositAttempt =
      depositType === 'usdc'
        ? createPreparedUsdcDepositAttempt({
            batch,
            livePreflight,
            preparedAt,
          })
        : createPreparedDepositAttempt({
            batch,
            livePreflight,
            preparedAt,
          });
...
export async function resetExpiredV10DepositPreparation({
  db,
  batchId,
  now = new Date(),
}) {
  ...
  const resetAt = assertValidDate(now, 'now');
  ...
  if (batch.status !== 'deposit_prepared') {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      ...
      { code: 'INVALID_RESET_DEPOSIT_BATCH_STATUS' },
    );
  }

  const preparedAttempt = batch.depositAttempt;

  if (
    !preparedAttempt ||
    preparedAttempt.status !== 'prepared' ||
    !preparedAttempt.groupId ||
    !preparedAttempt.unsignedTransactionHash
  ) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Only a complete unsigned prepared deposit attempt can be reset.',
      { code: 'INVALID_RESET_DEPOSIT_ATTEMPT' },
    );
  }

  if (
    batch.usdcDepositTxId !== null &&
    batch.usdcDepositTxId !== undefined
  ) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'A prepared deposit with a recorded deposit transaction cannot be reset.',
      { code: 'DEPOSIT_ALREADY_RECORDED' },
    );
  }

  if (
    batch.usdcDepositConfirmedAt !== null &&
    batch.usdcDepositConfirmedAt !== undefined
  ) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'A prepared deposit with recorded confirmation cannot be reset.',
  ...
  const updateResult = await settlementBatches.findOneAndUpdate(
    {
      ...
      'depositAttempt.groupId': preparedAttempt.groupId,
      'depositAttempt.unsignedTransactionHash':
        preparedAttempt.unsignedTransactionHash,
    },
    {
  ...
  if (!racedBatch) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      `Settlement batch ${String(
        batch._id,
      )} disappeared during expired deposit reset`,
      { code: 'BATCH_DISAPPEARED_DURING_DEPOSIT_RESET' },
    );
  }

  if (racedBatch.status === 'recipient_snapshot_prepared') {
    return Object.freeze({
      batchId: String(racedBatch._id),
      status: racedBatch.status,
      resetAt: new Date(racedBatch.updatedAt),
    });
  }

  throw new RevenueSettlementRecipientSnapshotValidationError(
    `Expired V10 deposit reset conditional update lost for batch ${String(
      batch._id,
    )}`,
    { code: 'DEPOSIT_RESET_CONDITIONAL_UPDATE_LOST' },
  );
}
...
function createSubmittedDepositAttemptFilter(
  prepared,
  persistedBatchId,
) {
  return {
    _id: persistedBatchId,
    status: 'deposit_prepared',
    usdcDepositTxId: null,
    usdcDepositConfirmedAt: null,
    'depositAttempt.operation': V10_DEPOSIT_OPERATION,
...
export async function persistSubmittedV10DepositAttempt({
  ...
  const updateResult = await settlementBatches.findOneAndUpdate(
    createSubmittedDepositAttemptFilter(prepared, batch._id),
    {
      $set: {
        status: 'deposit_submitted',
  ...
  const updateResult = await settlementBatches.findOneAndUpdate(
    createConfirmedDepositBatchFilter(batch, confirmedRecovery),
    {
      $set: {
        ...
        confirmedRound: confirmedRecovery.confirmedRound,
...
function assertPreparedCreatePayoutRoundAttempt({
  ...
  if (attempt.roundPayeesHash !== batch.roundPayeesHash) {
    ...
    const nextRoundId = assertSafeInteger(
      attempt.nextRoundId,
      'attempt.nextRoundId',
      { minimum: 1 },
    );
  ...
  const companionPaymentTransactionId = assertNonEmptyString(
    ...
    const { address: sender } = getAddressBytes(
      attempt.sender,
      'attempt.sender',
    );
    ...
    try {
      rebuilt =
      rebuildV10CreatePayoutRoundGroupFromUnsignedTransactions({
        unsignedTransactionsBase64: attempt.unsignedTransactionsBase64,
        appId: batch.revenuePoolAppId,
        poolKey: batch.poolKey,
        sender,
        roundPayees: batch.roundPayees,
      ...
      throw new RevenueSettlementRecipientSnapshotValidationError(
        'Payout submission unsigned transactions cannot be rebuilt as the exact V10 create_payout_round group',
        {
          code: 'INVALID_PAYOUT_SUBMISSION_UNSIGNED_GROUP',
          cause,
        },
      );
    }
      ...
      if (
        rebuilt.groupId !== attempt.groupId ||
        rebuilt.unsignedTransactionHash !==
          attempt.unsignedTransactionHash ||
        rebuilt.transactionIds.appCall !== appCallTransactionId ||
        rebuilt.transactionIds.companionPayment !==
          companionPaymentTransactionId ||
        rebuilt.roundBoxMbrMicroalgos !==
          attempt.roundBoxMbrMicroalgos ||
        rebuilt.appId !== batch.revenuePoolAppId ||
        rebuilt.poolKey !== batch.poolKey ||
        rebuilt.sender !== sender ||
        rebuilt.totalUsdcAtomicUnits !==
          batch.totalUsdcAtomicUnits ||
        rebuilt.nextRoundId !== nextRoundId ||
        rebuilt.roundPayeesHash !== batch.roundPayeesHash
      ) {
      throw new RevenueSettlementRecipientSnapshotValidationError(
        'Payout submission metadata does not match its persisted unsigned V10 payout-round group',
...
function assertSubmittedV10CreatePayoutRoundSubmission(batch) {
  if (!batch || typeof batch !== 'object') {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Persisted submitted payout batch must be an object',
      { code: 'INVALID_PERSISTED_BATCH' },
    );
  }

  if (batch.status !== 'payout_submitted') {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      `Persisted batch status must be "payout_submitted"; received "${String(
        batch.status,
      )}"`,
      { code: 'INVALID_PERSISTED_PAYOUT_SUBMISSION_STATUS' },
    );
  }

  const prepared = assertPreparedCreatePayoutRoundAttempt({
    batch: {
      ...batch,
  ...
  const submittedAt = assertValidDate(
    batch.payoutSubmissionAttempt?.submittedAt,
    'payoutSubmissionAttempt.submittedAt',
  );

  if (
    batch.payoutSubmissionAttempt?.confirmedAt !== null &&
    batch.payoutSubmissionAttempt?.confirmedAt !== undefined
  ) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Submitted payoutSubmissionAttempt.confirmedAt must be null',
      {
        code:
          'INVALID_SUBMITTED_PAYOUT_SUBMISSION_ATTEMPT_CONFIRMATION_STATE',
      },
    );
  }

  if (batch.revenueRoundTxId !== prepared.transactionIds.appCall) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Submitted payout batch revenueRoundTxId must match the durable app-call transaction ID',
      { code: 'PAYOUT_SUBMISSION_TRANSACTION_MISMATCH' },
    );
  }
  ...
  return Object.freeze({
    ...
    revenueRoundTxId: prepared.transactionIds.appCall,
    payoutSubmissionAttempt: Object.freeze({
      ...prepared,
      status: 'submitted',
      submittedAt,
      confirmedAt: null,
...
function createPreparedCreatePayoutRoundAttempt({
  ...
  return {
    ...
    nextRoundId: assertSafeInteger(
      unsignedGroup.nextRoundId,
      'unsignedGroup.nextRoundId',
      { minimum: 1 },
    ),

    sender: getAddressBytes(sender, 'sender').address,
...
export async function prepareCreatePayoutRoundSubmission({
  ...
  const liveRound = await readV10NextPayoutRoundId({
    algodClient,
    appId: distribution.revenuePoolAppId,
    poolKey: distribution.poolKey,
  });

  const frozenRoundPayees = distribution.payoutInstructions.map(
    (instruction) => ({
  ...
  const unsignedGroup = await buildUnsignedGroup({
    ...
    poolKey: distribution.poolKey,
    sender,
    roundPayees: frozenRoundPayees,
    totalUsdcAtomicUnits:
      distribution.totalUsdcAtomicUnits,
    nextRoundId: liveRound.nextRoundId,
    suggestedParams,
  });
  ...
  if (
    ...
    unsignedGroup.appId !== distribution.revenuePoolAppId ||
    unsignedGroup.poolKey !== distribution.poolKey ||
    unsignedGroup.sender !== sender ||
    unsignedGroup.totalUsdcAtomicUnits !==
      distribution.totalUsdcAtomicUnits ||
    unsignedGroup.nextRoundId !== liveRound.nextRoundId ||
    unsignedGroup.roundPayeesHash !==
      distribution.roundPayeesHash ||
    unsignedGroup.transactionCount !== 2 ||
    unsignedGroup.appCallTransactionIndex !== 0 ||
    unsignedGroup.companionPaymentTransactionIndex !== 1
  ) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      'Unsigned payout-round group does not match live and frozen payout facts',
      { code: 'PAYOUT_SUBMISSION_GROUP_MISMATCH' },
    );
  }
...
/**
 * Persists that the wallet submitted the exact durable prepared V10
 * create_payout_round group.
 *
 * This function does not rebuild the group, sign, broadcast, confirm,
 * re-snapshot holders, create another payout round, or distribute USDC.
 */
export async function persistSubmittedV10CreatePayoutRoundSubmission({
  db,
  batchId,
  submittedAt = new Date(),
}) {
  ...
  if (batch.status === 'payout_submitted') {
    return assertSubmittedV10CreatePayoutRoundSubmission(batch);
  }
  ...
  if (batch.status !== 'payout_prepared') {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      `Settlement batch ${String(
        batch._id,
      )} must have status "payout_prepared"; received "${String(
        batch.status,
      )}"`,
      { code: 'INVALID_PAYOUT_SUBMISSION_STATUS' },
    );
  }
  ...
  const updateResult = await settlementBatches.findOneAndUpdate(
    {
      _id: batch._id,
      status: 'payout_prepared',
      revenueRoundId: prepared.payoutRoundKey,
      revenueRoundTxId: null,
      'payoutSubmissionAttempt.operation':
        V10_CREATE_PAYOUT_ROUND_OPERATION,
      'payoutSubmissionAttempt.status': 'prepared',
      'payoutSubmissionAttempt.groupId': prepared.groupId,
      'payoutSubmissionAttempt.unsignedTransactionHash':
        prepared.unsignedTransactionHash,
      'payoutSubmissionAttempt.transactionIds.appCall':
        prepared.transactionIds.appCall,
      'payoutSubmissionAttempt.transactionIds.companionPayment':
        prepared.transactionIds.companionPayment,
      'payoutSubmissionAttempt.totalUsdcAtomicUnits':
        prepared.totalUsdcAtomicUnits,
      'payoutSubmissionAttempt.roundPayeesHash':
        prepared.roundPayeesHash,
      'payoutSubmissionAttempt.nextRoundId':
        prepared.nextRoundId,
      'payoutSubmissionAttempt.sender': prepared.sender,
    },
    {
      $set: {
        status: 'payout_submitted',
        revenueRoundTxId: prepared.transactionIds.appCall,
        'payoutSubmissionAttempt.status': 'submitted',
        'payoutSubmissionAttempt.submittedAt': normalizedSubmittedAt,
        updatedAt: normalizedSubmittedAt,
      },
    },
    {
  ...
  if (updatedBatch) {
    return assertSubmittedV10CreatePayoutRoundSubmission(updatedBatch);
  }
  ...
  if (!racedBatch) {
    throw new RevenueSettlementRecipientSnapshotValidationError(
      `Settlement batch ${String(
        batch._id,
      )} disappeared during payout submission persistence`,
      { code: 'BATCH_DISAPPEARED_DURING_PAYOUT_SUBMISSION' },
    );
  }

  if (racedBatch.status === 'payout_submitted') {
    return assertSubmittedV10CreatePayoutRoundSubmission(racedBatch);
  }

  throw new RevenueSettlementRecipientSnapshotValidationError(
    `V10 payout-round submission conditional update lost for batch ${String(
      batch._id,
    )}`,
    { code: 'PAYOUT_SUBMISSION_CONDITIONAL_UPDATE_LOST' },
  );
