...
test('routes exact pool proxy through explicit unallocated USDC preflight', async () => {
  const algodClient = createAlgodClient({
    poolBoxValue: encodePoolBox({
      proxyAddress,
    }),
  ...
  assert.equal(result.unsignedGroup.transactionCount, 2);
  assert.equal(
...
test('rejects unsupported deposit type before Algod reads', async () => {
  const algodClient = createAlgodClient();
...
test('accepts typed SDK Uint8Array application admin state', async () => {
