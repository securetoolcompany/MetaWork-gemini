...
// ── normalizeProduct (same as in your dialog) ──
...
export default function ProductDetailContent({
  productId,
  onDialogClose, // optional: called when you want to close the dialog (only used by dialog)
}) {
  ...
  // ── Keep selectedVariation in sync with color/size ──
  useEffect(() => {
          ...
          {creator && (
            <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-[#1e293b] to-[#0f172a] rounded-xl border border-white/10">
              ...
              <div className="flex gap-2">
                <Link href={`/aisle/${creator.username}`} onClick={() => onDialogClose?.(false)}>
                  <Button
                    variant="outline"
                    size="sm"
