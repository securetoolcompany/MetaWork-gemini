  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-6xl max-h-[95vh] md:max-h-[90vh] overflow-y-auto bg-[#020617] border-white/10 text-white p-4 md:p-8">
        <DialogHeader><DialogTitle className="sr-only">{productName}</DialogTitle></DialogHeader>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-12">
                  ...
                  {displayImages.length > 0 ? (
                    <>
                      ...
                      {displayImages.length > 1 && (
                        <>
                          <button onClick={() => setCurrentMockupIndex(prev => prev === 0 ? displayImages.length - 1 : prev - 1)} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 p-2 rounded-full"><ChevronLeft className="w-5 h-5" /></button>
                          <button onClick={() => setCurrentMockupIndex(prev => prev === displayImages.length - 1 ? 0 : prev + 1)} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 p-2 rounded-full"><ChevronRight className="w-5 h-5" /></button>
                        </>
                      )}
                    </>
                  ) : (
                    <div className="flex flex-col items-center justify-center h-full text-gray-500"><Package className="w-16 h-16 mb-2" /></div>
                  )}
                </div>
                {displayImages.length > 1 && (
                  <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
                    {displayImages.map((image, idx) => (
                      <button key={idx} onClick={() => setCurrentMockupIndex(idx)} className={`relative w-20 h-20 flex-shrink-0 rounded-lg border-2 ${currentMockupIndex === idx ? 'border-emerald-500' : 'border-white/20'}`}>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-baseline gap-3">
                      <span className="text-4xl font-black text-emerald-400">${parseFloat(productPrice).toFixed(2)}</span>
                      {hasDiscount && <span className="text-xl text-gray-500 line-through">${parseFloat(regularPrice).toFixed(2)}</span>}
                    </div>
                  </div>
                  {product.categories?.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {product.categories.map((cat, idx) => (
                        <button key={idx} onClick={() => handleCategoryClick(cat)} className="inline-flex items-center gap-1 px-3 py-1 bg-[#1e293b] border border-emerald-500/30 text-emerald-400 rounded-full text-sm hover:bg-emerald-500/20 transition-all">
                ...
                {creator && (
                  <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-[#1e293b] to-[#0f172a] rounded-xl border border-white/10">
                    <Avatar className="h-12 w-12 border-2 border-emerald-500/50"><AvatarImage src={creator.avatar} /><AvatarFallback>{creator.username?.[0]}</AvatarFallback></Avatar>
                    ...
                    <div className="flex gap-2">
                      <Link href={`/aisle/${creator.username}`} onClick={() => onOpenChange(false)}><Button variant="outline" size="sm" className="text-emerald-400 border-emerald-500/30"><MapPin className="w-4 h-4 mr-1" /> Aisle</Button></Link>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-4 pt-6 border-t border-white/10">
                  {!showShippingForm ? (
                    <div className="flex flex-col gap-3">
                      <Button onClick={handleAddToCart} disabled={cartLoading} className="w-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/50 py-6 font-bold transition-all">
            ...
            <div className="max-w-4xl mx-auto w-full pb-8">
              <h3 className="font-bold text-2xl mb-6 flex items-center gap-3"><MessageSquare className="w-6 h-6 text-emerald-500" /> Customer Reviews ({reviews.length})</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-[#1e293b] p-6 rounded-xl border border-white/10 h-fit">
                   ...
                   <div className="flex gap-1 mb-4">
                     {[1, 2, 3, 4, 5].map(s => <Star key={s} onClick={() => setUserRating(s)} className={`w-6 h-6 cursor-pointer ${s <= userRating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-600'}`} />)}
                   </div>
                   <Textarea value={userComment} onChange={(e) => setUserComment(e.target.value)} className="bg-[#0f172a] border-white/10 text-white mb-4 min-h-[120px]" />
                   <Button onClick={handleSubmitReview} disabled={submittingReview} className="w-full bg-emerald-500 text-black hover:bg-emerald-600 font-semibold"><Send className="w-4 h-4 mr-2" /> Submit</Button>
                </div>
                <div className="md:col-span-2 space-y-4">
                  ...
                  )) : <div className="text-center py-12 text-gray-500 border border-dashed border-white/10 rounded-xl">No reviews yet.</div>}
                </div>
              </div>
            </div>
              ...
              <div className="pt-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  {relatedProducts.map((p) => (
                    <button key={p.id} className="text-left group" onClick={() => { setProduct(null); router.push(`/products/${p.id}`); }}>
                      <div className="relative aspect-square rounded-xl overflow-hidden bg-gray-900 border border-white/10 mb-3 group-hover:border-emerald-500/50 transition-all">
                        {p.imageUrl && <Image src={p.imageUrl} alt={p.name} fill className="object-cover group-hover:scale-105 transition-transform" />}
                      </div>
          ...
          <div className="p-12 text-center text-gray-400 min-h-[50vh] flex items-center justify-center">Product not found.</div>
        )}
      </DialogContent>
    </Dialog>
  );
