...
import { Layers, Info, X, Loader2, ChevronUp, ChevronDown } from "lucide-react";
...
export default function DesignPropertiesPanel({
  ...
  const handleConfirmSave = useCallback(
    ...
    [pendingTitle, externalProductId, printfulTemplateId, selectedIPs, product, costAnalysis, selectedVariantId, router]
  );
  ...
  const handleSaveDraft = useCallback(
    ...
    [saving, onTriggerEdmSave, selectedIPs.length, printfulTemplateId, handleConfirmSave, pendingTitle, product?.name]
  );
  ...
  useEffect(() => {
    ...
    const loadVariants = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data.success && data.product?.variants) setVariants(data.product.variants);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (variants.length > 0 && !selectedVariantId) {
      const defaultVar = variants.find((v) => v.size === "L") || variants.find((v) => v.size === "M") || variants[0];
  ...
  return (
    <div className={cn(
      ...
      isMobile 
        ? "relative border-t rounded-t-2xl shadow-[0_-20px_50px_rgba(0,0,0,0.5)]" // Change fixed to relative
      ...
      {/* Main Content Area */}
      <div className={cn("flex-1 flex flex-col min-h-0", isMobile && !isExpanded && "hidden")}>
          ...
          {selectedIPs.length === 0 ? (
            ...
            <div className="space-y-2">
              {selectedIPs.map((ip, index) => (
                <div key={ip.id || index} className="relative flex items-center gap-3 p-2 rounded-md bg-slate-800/50 border border-slate-700">
                  <div className="w-8 h-8 bg-slate-950 rounded border border-slate-700 overflow-hidden shrink-0">
                    <img src={ip.imageUrl || ip.thumbnailUrl} alt="" className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-medium text-slate-200 truncate pr-6">{ip.title || ip.name}</h4>
                    <p className="text-[9px] text-green-400 font-mono">+${(Number(ip.licensingFee) || 2.00).toFixed(2)}</p>
                  </div>
                  <button onClick={() => onRemoveIP(ip.id)} className="text-slate-500 hover:text-red-400 p-1">
               ...
               {loadingVariants ? (
                  ...
                  <Select value={selectedVariantId ? String(selectedVariantId) : ''} onValueChange={(val) => setSelectedVariantId(Number(val))}>
                    ...
                    <SelectContent className="bg-slate-800 border-slate-700">
                      {variants.map((v, i) => (
                        <SelectItem key={v.id || v.variant_id || i} value={String(v.id || v.variant_id || i)} className="text-[10px]">
                          {v.size} - ${Number(v.retail_price || v.price).toFixed(2)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
               ) : <span className="font-mono text-slate-300">${costAnalysis.base.toFixed(2)}</span>}
            </div>
            
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-400">Est. Total</span>
              <span className="text-sm font-bold text-blue-400 font-mono">${costAnalysis.total.toFixed(2)}</span>
            </div>
          </div>
      ...
      {/* Save Dialog remains the same but with mobile-optimized Input */}
      <Dialog open={showNameDialog} onOpenChange={setShowNameDialog}>
        <DialogContent className="bg-slate-900 border-slate-700 text-slate-100 max-w-[90vw] rounded-xl">
          ...
          <div className="flex gap-2 mt-4">
            <Button variant="outline" onClick={() => setShowNameDialog(false)} className="flex-1 border-slate-700">Cancel</Button>
            <Button onClick={() => handleConfirmSave()} className="flex-1 bg-blue-600">Save</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
