...
MAX_CLAIM_SCAN_ROUNDS = Int(4)
MAX_CLAIM_PAID_ROUNDS = Int(4)
...
def approval_program():
    ...
    # ----------------------------------------------------------------
    # CLAIM REVENUE ALL  (REWRITTEN in V7 — T07)
    #
    # Accepts exactly 3 args: [method, ip_id, start_round_id].
		# Scans at most four consecutive rounds beginning at start_round_id.
		# Pays at most four eligible unclaimed entries in that page.
		# Additional pages are claimed with later start_round_id values.
    # ----------------------------------------------------------------
    cra_pkey      = ScratchVar(TealType.bytes)
    cra_pbox      = ScratchVar(TealType.bytes)
    cra_cur       = ScratchVar(TealType.uint64)
    cra_r         = ScratchVar(TealType.uint64)
    cra_rkey      = ScratchVar(TealType.bytes)
    cra_rbox      = ScratchVar(TealType.bytes)
    cra_n         = ScratchVar(TealType.uint64)
    cra_i         = ScratchVar(TealType.uint64)
    cra_found     = ScratchVar(TealType.uint64)
    cra_amt       = ScratchVar(TealType.uint64)
    cra_off       = ScratchVar(TealType.uint64)
    cra_match_off = ScratchVar(TealType.uint64)
    cra_total_clm = ScratchVar(TealType.uint64)
    cra_total_pay = ScratchVar(TealType.uint64)
    cra_paid_rnds = ScratchVar(TealType.uint64)
    cra_start_round = ScratchVar(TealType.uint64)
    cra_end_round = ScratchVar(TealType.uint64)

    on_claim_revenue_all = Seq(
        Assert(Txn.application_args.length() == Int(3)),
				Assert(Len(Txn.application_args[2]) == Int(8)),

        cra_pkey.store(pool_key(ip_id)),
        (lambda mv: Seq(mv, Assert(mv.hasValue()), cra_pbox.store(mv.value())))(
            App.box_get(cra_pkey.load())
        ),
        cra_cur.store(read_u64(cra_pbox.load(), CUR_ROUND_OFFSET)),

        # Pagination argument: [method, ip_id, start_round_id]
        cra_start_round.store(Btoi(Txn.application_args[2])),
        Assert(cra_start_round.load() > Int(0)),
        Assert(cra_start_round.load() <= cra_cur.load()),

        # Scan at most four consecutive round boxes:
        # start_round_id through min(start_round_id + 3, current_round_id).
        If(
						cra_cur.load() - cra_start_round.load()
						< MAX_CLAIM_SCAN_ROUNDS - Int(1)
				).Then(
						cra_end_round.store(cra_cur.load())
				).Else(
						cra_end_round.store(
								cra_start_round.load() + MAX_CLAIM_SCAN_ROUNDS - Int(1)
						)
				),

        cra_total_clm.store(read_u64(cra_pbox.load(), TOTAL_CLM_OFFSET)),
        cra_total_pay.store(Int(0)),
        cra_paid_rnds.store(Int(0)),
        cra_r.store(cra_start_round.load()),

        While(
            And(
                cra_r.load() <= cra_end_round.load(),
                cra_paid_rnds.load() < MAX_CLAIM_PAID_ROUNDS,
            )
        ).Do(
            cra_rkey.store(round_key(ip_id, cra_r.load())),
            (lambda mv: Seq(
                mv,
                If(mv.hasValue()).Then(
                    cra_rbox.store(mv.value()),
                    cra_n.store(read_u16(cra_rbox.load(), RND_NENTRIES_OFFSET)),
                    ...
                    While(
                        ...
                        cra_off.store(
                            RND_ENTRIES_OFFSET + cra_i.load() * RND_ENTRY_SIZE
                        ),
                        If(
                            Extract(cra_rbox.load(), cra_off.load(), Int(32))
                            == Txn.sender()
                        ).Then(
                            cra_match_off.store(cra_off.load()),
                    ...
                    If(
                        ...
                        InnerTxnBuilder.SetFields({
                            ...
                            TxnField.xfer_asset:     USDC_ASSET_ID,
                            TxnField.asset_receiver: Txn.sender(),
                            TxnField.asset_amount:   cra_amt.load(),
                            TxnField.fee:            Int(0),
                        }),
                        InnerTxnBuilder.Submit(),
                        App.box_replace(
                        ...
                        cra_total_pay.store(cra_total_pay.load() + cra_amt.load()),
                        cra_paid_rnds.store(cra_paid_rnds.load() + Int(1)),
                    ),
                )
            ))(App.box_get(cra_rkey.load())),
            cra_r.store(cra_r.load() + Int(1)),
        ),

        If(cra_total_pay.load() > Int(0)).Then(
            App.box_replace(
                cra_pkey.load(), TOTAL_CLM_OFFSET,
...
if __name__ == "__main__":
    raise RuntimeError(
        "Do not compile directly. Use deploy_v9.py with the Algorand Python SDK."
    )