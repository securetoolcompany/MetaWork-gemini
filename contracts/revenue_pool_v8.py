...
def approval_program():
    ...
    #
    # Accepts exactly 2 args: [method, ip_id]
    # Scans rounds 1..cur_round, pays any unclaimed entry for sender.
    # Bounded claim batch: process and pay at most seven eligible rounds per call.
    # Additional pending rounds are claimed in subsequent application calls.
    # ----------------------------------------------------------------
    ...
    on_claim_revenue_all = Seq(
        ...
        While(
            And(
                ...
                cra_paid_rnds.load() < Int(7),   # Pay at most seven eligible rounds per call
            )
        ).Do(
...
if __name__ == "__main__":
    raise RuntimeError(
        "Do not compile directly. Use deploy_v8.py with the Algorand Python SDK."
    )