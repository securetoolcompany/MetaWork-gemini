...
def approval_program():
    ...
    on_create_pool = Seq(
        ...
        # MBR: key = 2 ("p_") + Len(ip_id); value = 73 (header) + sh_count * 35
        box_mbr.store(
            Int(2500) + Int(400) * (
    ...
    rel_amt     = ScratchVar(TealType.uint64)
    cur_held    = ScratchVar(TealType.uint64)
    cur_unalloc = ScratchVar(TealType.uint64)

    on_release_held = Seq(
        Assert(Txn.application_args.length() == Int(3)),
        Assert(Txn.sender() == Global.creator_address()),
        rel_amt.store(Btoi(Txn.application_args[2])),
        Assert(rel_amt.load() > Int(0)),
        rh_pkey.store(pool_key(ip_id)),
        (lambda mv: Seq(mv, Assert(mv.hasValue()), rh_pbox.store(mv.value())))(
            App.box_get(rh_pkey.load())
        ),
        cur_held.store(read_u64(rh_pbox.load(), HELD_OFFSET)),
        cur_unalloc.store(read_u64(rh_pbox.load(), UNALLOCATED_OFFSET)),
        Assert(rel_amt.load() <= cur_held.load()),
        App.box_replace(
            rh_pkey.load(), HELD_OFFSET,
            Itob(cur_held.load() - rel_amt.load())
        ),
        App.box_replace(
            rh_pkey.load(), UNALLOCATED_OFFSET,
            Itob(cur_unalloc.load() + rel_amt.load())
        ),
        Approve(),
    )
    ...
    on_create_payout_round = Seq(
        ...
        # MBR: key = 4 ("rnd_") + Len(ip_id) + 8 (round_id); value = round_size
        rnd_mbr.store(
...
if __name__ == "__main__":
    raise RuntimeError(
        "Do not compile directly. Use deploy_v6.py with the Algorand Python SDK."
    )