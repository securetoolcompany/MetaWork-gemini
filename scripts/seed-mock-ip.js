/**
 * Seed Script: Mock IP Assets for Testing
 * * Run with: node scripts/seed-mock-ip.js
 */
...
// Mock IP Assets with owner names
const mockIPAssets = [
  {
    id: uuidv4(),
...
async function seedMockIP() {
  ...
  try {
    ...
    // Clear existing mock IP (optional - comment out to append)
    console.log('Clearing existing mock IP assets...');
    await db.collection('ip_assets').deleteMany({ minted: true, isPublic: true });
    
    // Insert mock IP assets
    console.log('Inserting mock IP assets...');
    const ipResult = await db.collection('ip_assets').insertMany(
      mockIPAssets.map(ip => ({
        ...ip,
    ...
    console.log(`Inserted ${ipResult.insertedCount} IP assets`);
    
    // Create revenue pool records
    console.log('Creating revenue pool records...');
    const revenuePools = mockIPAssets.map(ip => ({
      id: uuidv4(),
      ipAssetId: ip.id,
      appId: Math.floor(Math.random() * 900000000) + 100000000,
      ownerWallet: 'DEMO_WALLET_ADDRESS_FOR_MOCK_IP',
      totalDeposited: ip.totalRevenue,
      totalClaimed: ip.totalRevenue * 0.6,
      accumulatedRevenue: ip.totalRevenue,
      claimableAmount: ip.totalRevenue * 0.4,
      lastClaimDate: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
      createdAt: ip.createdAt,
      updatedAt: new Date()
    }));
    
    await db.collection('revenue_pools').deleteMany({ ownerWallet: 'DEMO_WALLET_ADDRESS_FOR_MOCK_IP' });
    const poolResult = await db.collection('revenue_pools').insertMany(revenuePools);
    console.log(`Inserted ${poolResult.insertedCount} revenue pools`);
